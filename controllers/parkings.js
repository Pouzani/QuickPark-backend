...
exports.getRecentlyVisitedParkings = async(req,res,next)=>{
    try {
        ...
        tickets.sort(function(ticket1,ticket2){ return new Date(ticket2.arrivalTime).getTime() - new Date(ticket1.arrivalTime).getTime()})

        for(let ticket of tickets ){
            const docRef = doc(db, "parkings", ticket.parkingId);
            const docSnap = await getDoc(docRef);
            let parking = new Parking(docSnap.data().parkingName,docSnap.data().spotNumber,docSnap.data().state,docSnap.id);
                parkings.push(parking.data);
        }

        res.status(200).json({success:true,operation:"get Recently Visited Parkings", data:{parkings}});
