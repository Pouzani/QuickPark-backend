...
exports.addCar = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, operation: "Add new car", data: newCar });
  } catch (error) {
...
exports.updateCar = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, operation: "update car", data: { carId } });
  } catch (error) {
    req.quickpark = { errorCode: error.code };
    next();
  }
...
exports.deleteCar = async (req, res, next) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, operation: "delete car", data: { carId } });
  } catch (error) {
    req.quickpark = { errorCode: error.code };
    next();
  }
...
exports.accessCarControl = async (req, res, next) => {
  try {
    ...
    for (let user of querySnapshot.docs) {
      const queryToGetAllCars = query(
        collection(db, "users", user.data().email, "cars")
      );
